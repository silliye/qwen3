...
class ACTMillion(nn.Module):
    def __init__(self, query_tokens_num, query_tokens_dim, input_dim, kv_input_dim, nums_layer, intermiddle_token_dim, head_dim, head_num, effect_clip):
        super().__init__()
    ...
    def forward(self, X_input, colossus_mask):
        ...
        single_user_seq_sum = torch.sum(colossus_mask, dim=-1, dtype=torch.int64)
        kv_seq_info = single_user_seq_sum # [usernum, seq]

        # [usernum, 10w, dim] [usernum, 10w bool] -> [token, dim]
        # [effect_tokens, dim]

        effect_tokens = torch.sum(single_user_seq_sum, dim=0, keepdim=False, dtype=torch.int64)
        # 20w > 10w
        if effect_tokens > self.effect_clip:
            # single_user_seq_sum : [7w, 5w, 8w]
            # new_single_user_seq_sum : [3.5w, 2.5w, 4w]
            new_single_user_seq_sum = single_user_seq_sum * (self.effect_clip / effect_tokens)  
            indexs = torch.arange(0, max_seq_len).unsqueeze(0)
            indexs_mask = indexs < new_single_user_seq_sum.unsqueeze(-1)
            kv_seq_info = new_single_user_seq_sum # [usernum, seq]

        colossus_mask = indexs_mask * colossus_mask
