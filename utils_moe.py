...
class Expert(BaseNetwork):
    def __init__(self, hidden_dim, expert_intermediate_dim, name="Expert"):
        super().__init__(name)

        self.gate = nn.Linear(hidden_dim, expert_intermediate_dim)
        self.up_proj = nn.Linear(hidden_dim, expert_intermediate_dim)
        self.down_proj = nn.Linear(hidden_dim, expert_intermediate_dim)
        self.selu = nn.SELU()
    
    def forward(self, x:Tensor):
        gate = self.gate(x)
        up = self.up_proj(x)
        return self.down_proj(self.selu(gate) * up)


class Router_with_Balance(nn.Module):
    def __init__(self, k, hidden_dim, expert_nums, lanmda_bias=0.001):
        super().__init__()
    ...
    def forward(self, x:Tensor):
        ...
        # [T, expert_num]
        init_weight = self.router(x).sigmoid()

        # [T, expert_num]
        select_weight:Tensor = init_weight + self.router_bias

        # [T, k]
        select_topk_probs, select_topk_indices = select_weight.topk(k=self.topk, dim=-1)
        ...
        # [B, k]
        init_weight_topk = torch.gather(init_weight, dim=-1, index=select_topk_indices)
        
        init_weight_topk = F.normalize(init_weight_topk, p=1, dim=-1)

        # [B, K] 一个是最后真正要乘的权重, 一个是选择了哪些token
        return init_weight_topk, select_topk_indices

class Qwen2DecoderCrossLayer(BaseNetwork):
    def __init__(self, hidden_dim, num_attention_heads, num_key_value_heads, 
                 shared_expert_nums, routed_expert_nums, expert_nums, expert_intermediate_dim, k,
                 intermediate_size, token_size, rope_theta, name="qwen_cross_decoder"):
        super().__init__(name)
        ...
        # MOE shared_expert_nums + routed_expert_nums = expert_nums
        ...
        self.router_bias = nn.Parameter(Tensor(self.expert_nums - self.shared_expert_nums))
        self.router = nn.Linear(hidden_dim, expert_nums-shared_expert_nums, bias=False)

        self.Router = Router_with_Balance(k, hidden_dim, expert_nums-shared_expert_nums)
    ...
    def forward(self, decoder_input, encoder_fusion, seq_mask=None, kv_mask=None):
        ...
        decoder_moe_input_normed = self.ffn_input_rms_norm(decoder_cross_attn_output)

        decoder_moe_output = decoder_cross_attn_output + self.qwen2MOE(decoder_moe_input_normed)
    ...
    def qwen2MOE(self, moe_input:Tensor):  
        # moe_input [B, seq, dim]

        B, seq, dim = moe_input.shape
        ...
        # # [B*seq, expert_nums]
        # routed_weight = self.router(moe_input).reshape([B*seq, -1])

        # routed_weight_ = routed_weight

        # routed_weight_soft = F.softmax(routed_weight_)

        # # [token_num, K] = [B*seq, K] 
        # probs, routed_weight_soft_topk_indice = routed_weight_soft.topk(self.topk, dim=-1)
        

        # # [B*seq, K]
        # routed_weight_reweithed = probs / probs.sum(dim=-1, keepdim=True)



        # B = 2, seq = 2, K = 2, expert_num = 4
        # routed_weight_soft_topk_indice : [ [0, 2], [0, 3], [1, 2], [0, 2]]
        # routed_weight_reweithed        : [ [0.9, 0.1], [0.4, 0.6], [0.8, 0.2], [0.5, 0.5]]
        
        routed_weight_reweithed, routed_weight_soft_topk_indice = self.Router(moe_input)
        
        moe_input = moe_input.reshape([B*seq, -1])

        for i in range(self.expert_nums - self.shared_expert_nums):
            
            # i == 0:  [[T, F], [T, F], [F, F], [T, F]]
            hit_mask = (routed_weight_soft_topk_indice.int() == i)
            ...
            # [3, D]
            moe_sparse_output = self.routed_experts[i](moe_sparse_input)

            # [3,] : [0.9, 0.4, 0.5] 广播乘法
            moe_sparse_output = moe_sparse_output * routed_weight_reweithed[token_indice, token_k_indice].unsqueeze(-1)

            # 这个结果需要累加到一个固定的结果中:
            # 第一个代表填充的维度, index_fill_(0, [0, 1, 3], [3,D])
            moe_total_output.index_add_(0, token_indice, moe_sparse_output)
        
        return moe_total_output.reshape([-1, seq, dim])

    def qwen2MOE_v2(self, moe_input:Tensor):
        # 展平为一维度  
        ...
        routed_weight_reweithed:Tensor = routed_weight_reweithed.view(-1)
        routed_weight_soft_topk_indice:Tensor = routed_weight_soft_topk_indice.view(-1)

        moe_input = moe_input.reshape([B*seq, -1])


        # [0, 0, 0, 1, 2, 2, 2, 3] sorted_expert_indice
        # [0, 2, 6, 4, 1, 5, 7, 3] init_token_indice
        sorted_expert_indice, init_token_indice = routed_weight_soft_topk_indice.sort()

        # [0, 0, 1, 1, 2, 2, 3, 3]
        candi_token_index = torch.arange(0, seq*B).repeat_interleave(self.topk)
        
        # [0, 1, 3, 2, 0, 2, 3, 1] real_expert_index 为当前排完序后token
        real_expert_index = candi_token_index[init_token_indice]

        # [0.9, 0.4, 0.5, ...]
        real_expert_weight = routed_weight_reweithed[init_token_indice]

        # [0, 0, 0, 1, 2, 2, 2, 3] 为专家 0*3 & 1 & 2*3 &3
        # [0, 1, 3, 2, 0, 2, 3, 1] 
        # token 013要算Expert0
        # token 2  要算Expert1
        # token 023要算Expert2
        # token 1  要算Expert3
        # 这里的数字都是下标 (token下标和expert下标)
        # 这里本质是token和expert的匹配问题

        # 之后将输入x拿过来 [4, dim] -> [8, dim]  重复K次
        # moeinput [4, dim]
        # [E0, E1, E3, E2, E0, E2, E3, E1]
        sorted_moe_input = moe_input[real_expert_index]

        # [3, 1, 3, 1]
        offset = torch.bincount(sorted_expert_indice, minlength=self.routed_expert_nums)

        start_index = 0
        for i, count in enumerate(offset.tolist()):
            if count == 0:
                continue
            end_index = start_index + count

            # [end-start, dim]
            sorted_moe_output = self.routed_experts[i](sorted_moe_input[start_index:end_index, :])
            
            sorted_moe_output = sorted_moe_output * real_expert_weight[start_index:end_index].unsqueeze(-1)
            # 将其分配到

            moe_total_output.index_add_(0, real_expert_index[start_index:end_index], sorted_moe_output)

            start_index = end_index
        
        return moe_total_output
         


    def qwen2MOE_v3(self, moe_input:Tensor):
        # 用大矩阵
        # 展平为一维度  
